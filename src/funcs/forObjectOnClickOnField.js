export const forObjectOnClickOnField = (event, arrDaysOfWeek) => {
  ...
  const certainDateGMT = new Date(Date.UTC(currentYear, currentMonth, currentDate, hourNumber));
  const startD = certainDateGMT.toISOString().substr(0, 10);
  const endD = endHour === 0
    ? new Date(Date.UTC(currentYear, currentMonth, currentDate + 1)).toISOString().substr(0, 10)
    : startD;
